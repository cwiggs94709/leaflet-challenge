...
function createMap(earthquakeData) {
    ...
    // Create a legend to display magnitude and depth information
    ...
    legend.onAdd = function() {
        ...
        for (let i = 0; i < depths.length; i++) {
          let color = markerColor(depths[i] + 1); // Get color for current depth
          div.innerHTML +=
              `<i style="background:${color}"></i> ${depths[i]}${(depths[i + 1] ? "&ndash;" + depths[i + 1] : "+")} km<br>`;
      }
      return div;
  };
